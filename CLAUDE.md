...
Flow: `/from-linear <id>` or `/spec-new feature <id>` → `spec-author` drafts (and creates a Linear Project + one Todo issue per affected repo) → you approve → `/spec-implement <id>` → `spec-implementer` builds in dependency order, running `/audit-contract`, `/trace-flag`, `/check-isolation` as gates before `in-review`, moving each repo's Linear issue Todo → In Progress → In Review → Done as work actually happens. Every issue created this way carries a severity (Linear's native Priority field) and a repo badge label (the `Repo: *` group) matching the repo it's scoped to; `spec-implementer`/`bug-fix` must never edit a repo other than the one an issue is labeled for. Templates: `specs/feature.spec.template.md`, `specs/module.spec.template.md`. For a narrowly-scoped bug fix or small enhancement that doesn't need a spec, use `/bug-fix` instead — flat Linear issue in the team backlog, no project. See `specs/spec-authoring-practices.md` for the underlying practices (check code before proposing new entities, evidence-tag claims, name cross-repo contract impact, Linear tracking, severity/labels).

**Assignee convention (revised 2026-08-13 — supersedes the old fixed repo→developer mapping below wherever the two conflict):** a Linear project or issue is assigned to **one single developer** per project/issue, decided by task scope, not by repo or tech stack. Before assigning, judge whether the work is a **single-repo task** (naturally owned end-to-end by one person) or a **multi-repo task** (a project/issue set spanning several repos, still owned by one person for the whole thing, not split per repo/stack). **Always ask the user who to assign the work to at creation time** (get the name) rather than auto-deriving it from any fixed mapping — the repo→developer table above is historical context for past work, not a standing default going forward.
