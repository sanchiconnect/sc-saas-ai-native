...
> **Decided 2026-07-27 (Linear SAN-66): target-setting explicitly deferred, not ready to commit to numbers yet.** Product owner confirmed this is a genuine "not yet" rather than a silent gap — every `[TBD — product owner]` row below stands as an intentional deferral, to be revisited when there's a basis to commit to real figures (e.g. after a baseline measurement period). This also covers the still-open "AI feature adoption rate" formula choice (flag-enabled vs. actual-usage vs. tracking both) below — not picked yet, deferred alongside the targets themselves.
...
5. ~~For the two "AI feature adoption rate" formula readings proposed above... what are the actual target numbers...~~ **Explicitly deferred 2026-07-27 (Linear SAN-66)** — product owner confirmed not ready to commit to numbers or pick a formula yet; not an oversight, a stated "not yet." See the KPI Framework's deferral note above. Revisit when there's a basis to commit (e.g. after a baseline measurement period).
