...
Flags: 29 (Project Types not declared in skill source for 29 skills)
...
**Phase:** 03-Planning (also active in Build phase for live context retrieval)
...
**Phase:** 03-Planning (invoked on-demand throughout the sprint)
...
**Purpose:** Primary implementation assistant for AI Builders during build days — generates spec-anchored code for a React/Python FastAPI/PostgreSQL stack with provenance headers, spec traceability IDs, convention compliance, and pre-delivery TrustFabric PII checks.
...
- Convention compliance self-check against .cursorrules
- TrustFabric PII pre-check before delivering code to Builder
- Ambiguity escalation to POD Lead with spec-ambiguity-escalation.log
...
**Inputs:**
  Mandatory:
    ...
    - artifacts/openspec.yaml: Acceptance criteria for the requirement
    - specs/design.md: Architectural patterns and naming conventions
    - specs/api.md: API contracts and request/response schemas
    - specs/database.md: Schema definitions and ORM patterns
    - artifacts/policy-catalogue.yaml: Compliance rail prompt for this task
    ...
    - TrustFabric flags: PII and data contract constraints
  Optional:
    - artifacts/ai-manifest.json: Existing component registry for duplication check

**Outputs:**
- Implementation code files with provenance headers
- spec-ambiguity-escalation.log: One entry per escalated ambiguity

**Dependencies:**
- KnowledgeMesh: Provides contextualised spec chunks
- TrustFabric: Provides PII constraints and data contract rules
- SecretShield: All context payloads pass through SecretShield before injection
...
- Ambiguous spec requirements must be escalated; do not generate code by guessing
- Works best with atomic, well-defined spec requirements

**Summary:** Generates spec-anchored, convention-compliant code for React/FastAPI/PostgreSQL tasks with provenance headers, PII checks, and ambiguity escalation to the POD Lead.
...
**Phase:** 05-Validate (also active in Operate phase for feedback loop contribution)
...
**Phase:** 04-Build (also active in Release and Operate phases)
...
**Phase:** 03-Planning (also active in Build phase for compliance rail injection)
...
**Phase:** 05-Validate (also active in Operate phase for runtime governance)
...
**Phase:** 03-Planning (also active in Build and Validate phases)
...
**Phase:** 04-Build (also active in Release phase for data governance attestation)
...
## Generation Summary
  Total skills discovered: 51
  Total entries generated: 51
  Entries with all fields complete: 22
  Entries with flagged fields: 29
  Unreadable skill files: 0

## Validation Results
  V-CAT-01 (one entry per skill): PASS — 51 skills discovered, 51 entries generated, no skill omitted
  V-CAT-02 (all fields present): FLAGS — 29 entries have Project Types flagged as MISSING (see Flagged Entries below)
  V-CAT-03 (no duplicate names): PASS — all 51 Skill Names are unique
  V-CAT-04 (valid dependencies): PASS — all dependency references resolve to skills present in the inventory
  V-CAT-05 (no unsupported assumptions): PASS — all field values traced to source skill file content; no assumptions introduced
  V-CAT-06 (missing fields flagged): confirmed — all 29 missing Project Types fields carry the ⚠️ MISSING flag with reason
  V-CAT-07 (catalog_metadata.md version matches skill_catalog.md): PASS — both files carry Catalog Version 1
