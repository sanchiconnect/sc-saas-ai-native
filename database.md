...
| `sanchiconnect-saas-tenants-admin` | Medoo, one connection only (the per-tenant-lookup code is present, commented out, deliberately disabled) | Same `tenant_users`/`global_settings`/`spa_*` tables as above, plus its own AI-Credits CRUD (`grants`/`task_rates`/`packages`/`orders` — see §3), plus (via `modules/scrapper.php`) ad-hoc direct connections into **every individual tenant's own business DB**, reading `tenant_users.database_password` in plaintext to do so. Also the actual writer of `organizations` (surfaced as "Clients") via its generic CRUD engine — confirmed 2026-07-17 after finding a stale claim elsewhere that `sc-saas-admin` writes this table; it doesn't. `subscriptions` (`organization_id`/`tenant_id` both NOT NULL, FK'd to `organizations`/`tenant_users`; `plan_name`/`plan_users`/`plan_description`/`start_date`/`end_date`/`suspend_date`/`notes`/`active`) is reachable the same way but has no dedicated onboarding flow yet outside the generic Add/Edit screen. |
